...
このZIPはClaude Project実機テスト用の完全パッケージです。

- `CLAUDE_PROJECT_INSTRUCTIONS.md`: Project Instructionsへ貼り付け
- `knowledge/`: Project Knowledgeへ全ファイルを追加
- `contracts/`: Project Knowledgeへ全JSON Schemaを追加
- `templates/`: Project Knowledgeへ全ファイルを追加
- `examples/`: 任意。初回テスト時の入力例

`knowledge/`はリポジトリ内の分散資産をClaude向けに8ファイルへ統合してあります。リポジトリの設計内容を省略したREADMEだけのパッケージではありません。
